...
namespace
{
    ...
    namespace tk
    {
        ...
        public:
            bool getIsInitializedProperly() const
            {
            ...
            spline(const std::vector<Vec2> &vec, const BolideDescriptor &bolide, spline_type type = cspline,
                   bool make_monotonic = false,
                   bd_type left = second_deriv, double left_value = 0.0,
                   bd_type right = second_deriv, double right_value = 0.0) : m_type(type),
                                                                             m_left(left), m_right(right),
                                                                             ...
                                                                             m_made_monotonic(false)
            {
                ...
                {
                    Vec2 pointShifted = point - bolide.getPosition(); // shift
                    pointShifted.rotate(bolide.getTheta());
                    X.push_back(pointShifted.x);
                    Y.push_back(pointShifted.y);
                    if (prevX >= pointShifted.x)
                    {
                        isInitializedProperly = false;
                        return;
                    }
                    prevX = pointShifted.x;
                }
