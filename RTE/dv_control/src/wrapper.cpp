...
void Controller::publishControlCommand(double steering_angle,double torque_request)
{
    ...
    if(!all_path_recived){
        ...
        controlMsg.torque_fl = 0.0f;
        controlMsg.torque_fr = 0.0f;
        controlMsg.torque_rl = 0.0f;
        controlMsg.torque_rr = 0.0f;

        pub_control.publish(controlMsg);

    } else {
        // torque_request is in [-1,1] (throttle percentage). Convert to [%] like before.
        const float tq_pct = static_cast<float>(torque_request * 100.0);

        controlMsg.movement = 0.0f; // ignored in torque mode
        controlMsg.steeringAngle_rad = static_cast<float>(
            std::clamp(steering_angle, param_.get("min_delta"), param_.get("max_delta"))
        );
        controlMsg.finished = false;
        ...
        // 4x4 torque command: for now same on each wheel (controller still outputs a single throttle)
        controlMsg.torque_fl = tq_pct;
        controlMsg.torque_fr = tq_pct;
        controlMsg.torque_rl = tq_pct;
        controlMsg.torque_rr = tq_pct;
...
double Controller::acc_to_throttle_percentage(double a_desired)
{
    ...
    double torque_required = F_required * wheel_radius/4.0; // Moment obrotowy wymagany na kołach
