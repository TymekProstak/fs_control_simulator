// ===============================
// mpc_interface.cpp (UPDATED)
// - spójne z hpp
// - używa v_ref0 = velocity_ref[0] (fallback: param v_target)
// - w całym predykowaniu/liniaryzacji używa TYLKO v_ref0
// ===============================
...
// ============================================================
// Continuous dynamics (atan2 slip) — uses v_ref0
// ============================================================
    ...
    const ParamBank& param)
{
    ...
    xdot(0) = vy * c_epsi + v * s_epsi;
    xdot(1) = r - kappa * v;
    xdot(2) = (Fyf * c_delta + Fyr) / m - v * r;
...
// ============================================================
// reset_initial_guess — uses v_ref0
// ============================================================
void MPCInterface::reset_initial_guess(const MPC_State& x0, double v_ref0)
{
    ...
    for (int i = 0; i <= N; ++i) {
        const double t = i * dt;
        const double predicted_ey = current_ey + (v * std::sin(current_epsi)) * t;
...
// ============================================================
// Jacobian — uses v_ref0 (frozen)
// Signature matches hpp: (x, v_ref0, Ac, Bc)
// ============================================================
    ...
    Eigen::Matrix<double, NX, NU>& Bc)
{
    ...
    const double datan_f_dyf = v / denom_f;
    const double datan_r_dyr = v / denom_r;
...
// ============================================================
// LTV matrices — signature matches hpp:
// (x0, kappa_vec, vref_vec)
// BUT: uses ONLY v_ref0 = vref_vec[0] (frozen)
// ============================================================
                                         ...
                                         const std::vector<double>& vref_vec)
{
    ...
    if (!vref_vec.empty() && std::isfinite(vref_vec[0])) v_ref0 = vref_vec[0];
    ...
    for (int k = 0; k < N; ++k)
    {
        ...
        calculate_continuous_jacobian(x_curr, v_ref0, Ac_cont, Bc_cont);
        ...
        const Eigen::Matrix<double, NX, 1> x_next =
            rk4_step_consistent(x_curr, u_k, kappa, v_ref0, dt, param_);
        x_traj[k + 1] = x_next;
...
// ============================================================
// SOLVE — signature matches hpp
// - v_ref0 = velocity_ref[0] (fallback: param v_target)
// - reset + LTV use ONLY v_ref0
// ============================================================
