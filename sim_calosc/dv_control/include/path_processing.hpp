...
struct PathProcessResult
{
    ...
    // NEW: yaw polilinii w punkcie rzutu auta + signed dist do polilinii
    ...
    double normal_distance_to_path)
{
    ...
    r.mpc_eligible = false;      // open path → geo-only
    r.all_path_eligible = false;
...
// =====================================================
// Sampling: spline -> polyline with kappa
// =====================================================
    ...
    const ParamBank& P)
{
    ...
    // yaw + signed dist liczę do polilinii (segment AB)
    ...
    const ParamBank& P)
{
    ...
    // yaw + signed dist liczę z polilinii po dobudowach (najbliższy segment)
    const int ci = find_closest_segment_index(poly.path, q);
    if (ci < 0 || ci + 1 >= (int)poly.size()) return makeGeoInvalidResult();

    const double yaw_vehicle_projection = yawFrom2Pts(poly.path[ci], poly.path[ci + 1]);
    const double normal_distance_to_path = signedDistPointToSegment(poly.path[ci], poly.path[ci+1], q);
    ...
    const ParamBank& P)
{
    ...
    // (zostawiam jak było u Ciebie — zakomentowane)
    ...
    const State& bolide_state)
{
    ...
    // ---- NEW: yaw + signed dist do POLILINII (tu: polilinia z samplowania splajnu) ----
    {
        const double ds_poly = std::max(1e-3, P.get("distance_between_interpoleted_points"));
        Poly_with_kappa poly_track = sample_open_spline_to_poly_with_kappa(spline_closed, ds_poly);

        const int ci = find_closest_segment_index(poly_track.path, q);
        if (ci >= 0 && ci + 1 < (int)poly_track.size()) {
            out.yaw_vehicle_projection = yawFrom2Pts(poly_track.path[ci], poly_track.path[ci+1]);
            out.normal_distance_to_path = signedDistPointToSegment(poly_track.path[ci], poly_track.path[ci+1], q);
            out.yaw0 = out.yaw_vehicle_projection; // legacy
        } else {
            // fallback
            out.yaw_vehicle_projection = spline_closed.getYaw(s0);
            out.normal_distance_to_path = 0.0;
            out.yaw0 = out.yaw_vehicle_projection;
        }
    }
