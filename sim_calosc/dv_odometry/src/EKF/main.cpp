...
{
    ros::init(argc, argv, "kalman_filter_kinematic_node");
    ...
    try
    {
        ...
        kalman_filter_kinematic::ParamBank param_bank =
            kalman_filter_kinematic::build_param_bank(J);

        kalman_filter_kinematic::EKFWrapper wrapper(nh, pnh, param_bank);
        wrapper.spin();
    }
    catch (const std::exception& e)
    {
        ROS_ERROR_STREAM("[kalman_filter_kinematic_node] fatal error: " << e.what());
        return 1;
    }
