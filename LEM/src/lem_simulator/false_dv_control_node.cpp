...
int main(int argc, char** argv) {
    ...
    while (ros::ok()) {
        ...
        msg.move_type =  static_cast<bool>(true);
        msg.movement = static_cast<float> (3.0)  ; // 20 procent momentu
        msg.steeringAngle_rad = - amplitude * std::sin(2 * M_PI * frequency * ros::Time::now().toSec());
        //msg.steeringAngle_rad = static_cast<float>(0.0); // zakomentować tę linię, aby włączyć oscylacje
